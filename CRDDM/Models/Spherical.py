...
class SphericalDiffusionModel:
    ...
    def joint_lpdf(self, rt, theta, drift_vec, ndt, threshold, decay=0, threshold_function=None, dt_threshold_function=None, s_v=0, s_t=0, sigma=1):
        ...
        if self.threshold_dynamic == 'fixed':
            ...
            g_z, T = ie_fpt_linear(threshold, decay, 3*sigma**2, 0.000001, sigma=2*sigma**2, dt=0.02, T_max=T_max)
            fpt_z = np.interp(tt, T, g_z)
        elif self.threshold_dynamic == 'exponential':
            a = threshold * np.exp(-decay*tt)
            g_z, T = ie_fpt_exponential(threshold, decay, 3*sigma**2, 0.000001, sigma=2*sigma**2, dt=0.02, T_max=rt.max())
            fpt_z = np.interp(tt, T, g_z)
        elif self.threshold_dynamic == 'hyperbolic':
            a = threshold / (1 + decay*tt)
            g_z, T = ie_fpt_hyperbolic(threshold, decay, 3*sigma**2, 0.000001, sigma=2*sigma**2, dt=0.02, T_max=rt.max())
            fpt_z = np.interp(tt, T, g_z)
            ...
            g_z, T = ie_fpt_custom(threshold_function2, dt_threshold_function2, 3*sigma**2, 0.000001, sigma=2*sigma**2, dt=0.02, T_max=rt.max())
            fpt_z = np.interp(tt, T, g_z)
